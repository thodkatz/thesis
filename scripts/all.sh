...
nault_multi() {
    for batch in {0..10}; do
        for model in vidr-multi; do
            ...
            $HELPER $REPO/scripts/main.py --batch $batch --model $model --dataset nault-multi --perturbation tcdd $DEBUG --experiment scgen
        done
    done
}
...
nault_multi_liver() {
    for batch in {0..5}; do
        for model in vidr-multi; do
            echo "Model: $model, Batch: $batch"
            $HELPER $REPO/scripts/main.py --batch $batch --model $model --dataset nault-liver-multi --perturbation tcdd $DEBUG --experiment scgen
        done
    done
}
