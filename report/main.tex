...
Firstly, I would like to sincerely thank the supervisor Prof. Pericels Mitkas, for giving me this opportunity to collaborate with him and to conduct the thesis in this very interesting research area guided by his supervision and scientific support. This collaboration allowed me to widen my horizons, and gave me the chance to unravel a unique blend of biology and engineering, while extending the curriculum. Additionally, it has eventually pulled my attention, and inspired me to continue working on this developing research area.
...
Furthermore, I would like to sincerely thank the bioinformatics team of Institute of Applied Biosciences (INAB) of the Center for Research and Technology Hellas (CERTH), and specifically the leader of the team Mr. Fotis E. Psomopoulos, and the members George Gavriilidis, Vasileios Vasileiou, and Aspa Orfanou. The team has introduced me to the area of bioinformatics and supported in a multifaceted way throughout my diploma thesis, providing technological expertise and invaluable time for productive conversations, that have assisted me to navigate the research field.
...
I would like to mention as well, that this work won't be possible without the access of the computational resources of the European Molecular Biology Laboratory (EMBL) in Heidelberg. Thus, I would like to sincerely thank Dr. Anna Kreshuk, leader of the group of machine learning for bioimage analysis, for giving me the opportunity to be member of the team as a software engineer for the platform ilastik\footnote{\url{https://www.ilastik.org/}}, and for the access to the High-Performance Computing infrastructure to run my experiments.
...
Finally, I would like to thank my family and my friends for their emotional support throughout my research trajectory in Greece and abroad. 
...
Computational work has become an indispensable part of the biological research. Bioinformatics, the field that employs computational tools to analyze biological data, emerged in the 1960s with an initial focus on protein sequence analysis. In the modern era, driven by technological advances and the development of \gls{ngs} \cite{gauthier2019brief}, the field has expanded significantly to include large-scale sequencing of genetic material at the cellular level. Consequently, machine learning has become a natural and powerful approach for extracting knowledge from these vast and complex datasets \cite{libbrecht2015machine}.  


\subsection{Brief description of the problem}

%Single-cell technologies have made it possible to study biological heterogeneity at the cellular level, opening new avenues for understanding cellular mechanisms and their responses to perturbations. However, the perturbation space is vast and multidimensional, making it infeasible to investigate every possible condition in a laboratory setting due to cost and time constraints \cite{heumos2023best,kana2023generative}. This limitation has motivated the development of computational methods aimed at modeling this space, therefore enabling inference in novel in silico scenarios that simulate experimental settings.
%The field concerned with the characterization and prediction of biological responses to external influences (gene knockouts, drug dosages, or environmental changes), is known as perturbation modeling. This area plays a crucial role in elucidating disease mechanisms and identifying potential therapeutic targets \cite{jiMachineLearningPerturbational2021}.
...
\subsection{Objective of the diploma thesis}
The complex nature of biological responses to perturbations can be decomposed into related sub-objectives, enabling simplification and improved interpretability of the underlying processes.
\gls{mlt} provides a suitable framework for exploiting such a decomposition, as it allows multiple tasks to be trained jointly. By learning shared representations across tasks, \gls{mlt} can enhance predictive performance and improve generalization robustness \cite{ruderOverviewMultiTaskLearning2017}.
Thus, this paradigm can be a promising approach to resolve the challenges of the single-cell perturbation modeling.
However, despite its advantages, its application in this field remains unexplored. In this work, we attempt to fill this gap, investigating its potential.
...
Initially, we attempted to investigate which subtasks could be decomposed from the overall task of perturbation modeling, so they can be formulated within a multi-task learning setup. Then, we have focused to the applications of single-cell data, and specifically the prediction of the gene expression after a perturbation.

To utilize the multi-task learning paradigm, we have developed an innovative \gls{mlt} autoencoder architecture capable of predicting the single-cell transcriptomic profile for a set of perturbations by applying one of them. The perturbation is modeled as a vector that encodes its type, and it is integrated into the architecture of the autoencoder through \gls{film} layers \cite{dumoulin2018feature-wise}. The FiLM method consists of a combination of additive and multiplicative transformations, and it is applied to every node in the layer of interest.

The architecture is evaluated in three different scenarios for predicting perturbed transcriptomic profiles and is compared with corresponding methods proposed in the literature. Initially, the performance of a single-perturbation, \gls{ifnb}, experiment is examined in human \gls{pbmc} \cite{kanaGenerativeModelingSinglecell2023}. Regarding the second scenario, the architecture is evaluated in a multi-perturbation setting, through the administration of the drug \gls{tcdd} at different dosages in mice \cite{nault2021single,nault2022benchmarking}. Finally, the scenario of a single-perturbation, \gls{lps}, across different species (mouse, rat, rabbit, pig) is investigated, aiming to introduce an additional axis of variation with respect to the species, extending the condition (control, perturbed) specific variation of the previous scenarios.

\subsection{Contents}

The diploma thesis consists of five chapters. Following the introduction in Chapter 1, Chapter 2 presents the theoretical background. In this chapter, we analyze the technical challenges of perturbation modeling at the single-cell level and review the methods proposed in the literature to address them. Additionally, we introduce the fundamental theory of multi-task learning and discuss its potential applications in molecular biology.
Chapter 3 focuses on the task of \gls{ood} detection, which constitutes the main field of interest of this work. In this chapter, we explore the application of a multi-task learning approach to this problem and describe the core \gls{mlt} architecture developed in this thesis, along with its variants.
In Chapter 4, we present and analyze the experimental results, comparing the proposed multi-task learning methods with state-of-the-art approaches reported in the literature. Finally, Chapter 5 summarizes the conclusions of the research and outlines directions for future work. The thesis concludes with information on code availability, followed by the appendix and the glossary. 
...
The advancement of technology in the sequencing of genetic material, and specifically \gls{ngs}, shaped two core approaches of detecting gene expressions within a biological sample. The first, named as bulk RNA-seq, is capable of counting the transcriptome of a cell culture, recording an average value of the overall gene expression. On the other hand, the second approach, known as \gls{sc}, focuses on the gene expression of individual cells, that enhances the transcriptomic resolution and unravels the cellular heterogeneity. Therefore, single-cell technologies enable the elucidation of cellular responses while preserving cell-specific information that is lost through averaging in bulk RNA-seq experiments. \cite{10xBulkVsSingleCell2025}. 

\begin{figure}[h!]
    \centering
    \includegraphics[width=.8\textwidth]{bulk_vs_single.png}
    \caption{Comparison of sequencing technologies: single cell RNA-seq and bulk RNA-seq\cite{mbzuai2024future}}
\end{figure}
...
\subsection{Literature models for the out-of-distribution (OOD) detection task in single-cell data}
...
Although scButterfly isn't primarily designed for perturbation modeling, the study has demonstrated its potential, by treating control and perturbed expression profiles as two modalities. One of its limitations is the narrow evaluation scope, as it has been tested only on the case of human \gls{pbmc} stimulated by \gls{ifnb} \cite{kanaGenerativeModelingSinglecell2023}.
...
\begin{figure}[h!]
    ...
    \caption[Illustration of the multi-task architecture]{Illustration of the multi-task architecture (created in  \url{https://BioRender.com}}). The encoder is shared across all tasks, while the decoder is conditioned by the task-specific FiLM layers. In this example, only two film layers are integrated in the first two hidden layers of the decoder. The FiLM gener ator takes as input a conditioning representation of the perturbations (e.g. one-hot encoding), and outputs the modulation parameters $\gamma$, and $\beta$ for the FiLM layers. 
\end{figure}
% I have to document the losses


We have explored several variations of this approach, all of which maintain the decoder architecture with the inclusion of FiLM-based conditioning for all the hidden layers of the decoder. These variations can be split to three main groups, a) adversarial autoencoders, b) optimal transport, c) \gls{vae}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{basic_flowchart.png}
    \caption{Flowchart for one iteration of training of the MTAe architecture}
\end{figure}
...
In our adversarial autoencoder variations, we enforce structure in the latent space via an adversarial loss. The architecture builds on the previously described autoencoder with \gls{film} layers, extended with a discriminator. The discriminator is trained to distinguish between encoded latent vectors and samples from a target distribution, while the encoder is trained adversarially to fool the discriminator. This encourages the latent space to match the desired target distribution.
...
, where $D(z_i)$ is the discriminator's prediction for the $i$th latent vector $z_i$, and $y_i$ is the true label.
...
\begin{figure}[h!]
    \includegraphics[width=.9\textwidth]{mtadv.png}
    \caption{Illustration of the MTAeAdv architecture}
\end{figure}

\begin{figure}[h!]
    \includegraphics[width=.9\textwidth]{mtadvg.png}
    \caption{Illustration of the MTAeAdvG architecture}
\end{figure}



\begin{figure}[h!]
    \includegraphics[width=.9\textwidth]{gaussian_flowchart.png}
    \caption{Flowchart for one iteration of training of the MTAeAdvG and the MTAeAdv architectures}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Illustration of the MTAeOT architecture}
\end{figure}

\begin{figure}[h!]
    \includegraphics[width=\textwidth]{ot_flowchart.png}
    \caption{Flowchart for one iteration of training of the MTAeΟΤ architecture}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Illustration of the MTAePlusOT and MTVaePlusOT architectures}
    \label{fig:mtotfinetune}
\end{figure}

\begin{figure}[h!]
    \includegraphics[width=\textwidth]{mtvae.png}
    \caption{Illustration of the MTVae architecture}
\end{figure}


\begin{figure}[h!]
    \includegraphics[width=\textwidth]{vae_flowchart.png}
    \caption{Flowchart for one iteration of training of the MTVae architecture}
\end{figure}
...
We have tested the models across all the available cell types, training a model from scratch for each use case. To test each model, we held out the perturbed gene expression of the cell type of interest and used the rest to train (\cref{fig:eval_pbmc,fig:eval_pbmc_distance}).
...
\begin{figure}[h!]
    ...
    \caption{$R^2$ for the top 20 \gls{hvg}}
    \end{subfigure}    
\end{figure}
...
\begin{figure}[h!]\ContinuedFloat
    ...
    \caption{$R^2$ for the top 100 \gls{hvg}}
    \end{subfigure}        
    
    \caption{Baseline metrics per cell type (Kang et al. \cite{kanaGenerativeModelingSinglecell2023})}
    \label{fig:eval_pbmc}
\end{figure}
...
\begin{figure}[p]\ContinuedFloat
    ...
    \caption{Distance metrics per cell type  (Kang et al. \cite{kanaGenerativeModelingSinglecell2023})}
...
\begin{figure}[p]\ContinuedFloat
    ...
    \caption{UMAP train test split (Kang et al. \cite{kanaGenerativeModelingSinglecell2023})}
...
\begin{figure}[h!]
    ...
    \caption{Violin plots gene expression distributions for the ISG15 gene CD4T cell type (Kang et al. \cite{kanaGenerativeModelingSinglecell2023})}
...
\begin{table}[h!]
    ...
    \bottomrule
    \end{tabularx}}
    \caption{Averaged metrics across all cell types (Kang et al. \cite{kanaGenerativeModelingSinglecell2023})}
    \label{tab:eval_pbmc}
\end{table}
...
In contrast, the baseline models, which do not support conditioning on multiple perturbations, required to train a separate model for each dosage. An exception is scVIDR, which is designed to handle multiple dosage levels within a single model.
...
\begin{figure}[h!]\ContinuedFloat
    ...
    \caption{Baseline metrics per cell type (Nault et al. \cite{nault2021single,nault2022benchmarking})}
    \label{fig:eval_nault}
\end{figure}
...
\begin{figure}[p]\ContinuedFloat
    ...
    \caption{Distance metrics per cell type (Nault et al. \cite{nault2021single,nault2022benchmarking})}
...
\begin{figure}[h!]\ContinuedFloat
    ...
    \caption{Baseline metrics per dose (Nault et al. \cite{nault2021single,nault2022benchmarking})}
...
\begin{figure}[p]\ContinuedFloat
    ...
    \caption{Distance metrics per dose (Nault et al. \cite{nault2021single,nault2022benchmarking})}
...
\begin{figure}[p]\ContinuedFloat
    ...
    \caption{UMAP train test split for all the doses (Nault et al. \cite{nault2021single,nault2022benchmarking})}
...
\begin{figure}[p]\ContinuedFloat
    ...
    \caption{UMAP train test split for the dose of $30 \mu g/kg$ (Nault et al. \cite{nault2021single,nault2022benchmarking})}
...
\begin{table}[h!]
    ...
    \bottomrule
    \end{tabularx}}
    \caption{Averaged metrics across all cell types Nault et al. \cite{nault2021single,nault2022benchmarking}}
    \label{tab:eval_nault}
\end{table}
...
\begin{figure}[h!]\ContinuedFloat
    ...
    \caption{Baseline metrics per species (Hagai et al. \cite{hagai2018gene})}
...
\begin{figure}[p]\ContinuedFloat
    ...
    \caption{Distance metrics per species (Hagai et al. \cite{hagai2018gene})}
...
\begin{table}[h!]
    ...
    \bottomrule
    \end{tabularx}}
    \caption{Averaged metrics across all cell types Hagel et al. \cite{hagai2018gene}}
