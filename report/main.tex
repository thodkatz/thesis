...
I should explore scVIDR. There is also a very new deep generative model including perturbations tasks https://github.com/mcgilldinglab/scCross (published 29 July). Multi cpa as well integrates multi-omics for perturbation modeling. Since we want to focus on multi-modal multi-task framework, we need to explore the multi-modal frameworks first and then later on the multi-task learning.
...
\begin{itemize}
  \itemsep -0.1em
  \item Out of distribution prediction of perturbed gene expression profile. Perturbations can be either chemical (drug doses) or genetic (gene knockouts visa CRISPR). This out of distribution could mean to predict the perturbed gene expression profile for a perturbation on a different cell type that the model has not seen. Note that the model is trained on cells exposed to one perturbation, so we can't generalize on unseen perturbations, but unseen cells. Sometimes though predicting perturbation that the model has seen to a different cell type is characterized as unseen perturbation, that could lead to confusion (at least for me). Effectively it is an unseen perturbation, since you have never seen the combon new cell type + perturbation, but you have seen some cell types + perturbation.
